...
This can be prevent if either sticky sessions were enabled, so the client would have connected to server A again, or if a shared locking mechansim between the servers would be required.
